...
def get_average(pixels):
    ...
    green_sum = 0
    blue_sum = 0
    # to find the average of red, green, blue
    for pixel in pixels:
        red_sum += pixel.red
        green_sum += pixel.green
        blue_sum += pixel.blue
    rgb_avg_l = [red_sum // len(pixels), green_sum // len(pixels), blue_sum // len(pixels)]
    return rgb_avg_l
...
def get_best_pixel(pixels):
    ...
    best_pixel = pixels[0]
    for pixel in pixels:
        if get_pixel_dist(pixel, get_average(pixels)[0], get_average(pixels)[1], get_average(pixels)[2]) < get_pixel_dist(best_pixel, get_average(pixels)[0], get_average(pixels)[1], get_average(pixels)[2]):
            best_pixel = pixel
    return best_pixel
